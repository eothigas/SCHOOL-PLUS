@extends('layouts.app')
@section('title', 'Editar Disciplina')

@section('content')
<div class="sp-page-hdr">
    <div>
        <h1 class="sp-page-hdr-title">Editar Disciplina</h1>
        <div class="sp-page-hdr-sub">{{ $disciplina->nome }}</div>
    </div>
    <a href="{{ route('disciplinas.index') }}" class="btn btn-outline-secondary">
        <i class="bi bi-arrow-left me-1"></i>Voltar
    </a>
</div>

<form method="POST" action="{{ route('disciplinas.update', $disciplina) }}">
    @csrf @method('PUT')
    <div class="sp-card">
        <div class="row g-3">
            <div class="col-md-6">
                <label class="form-label">Curso *</label>
                <select name="curso_id" class="form-select @error('curso_id') is-invalid @enderror">
                    @foreach($cursos as $curso)
                    <option value="{{ $curso->id }}" @selected(old('curso_id', $disciplina->curso_id) == $curso->id)>{{ $curso->nome }}</option>
                    @endforeach
                </select>
                @error('curso_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="col-md-4">
                <label class="form-label">Nome *</label>
                <input type="text" name="nome" class="form-control @error('nome') is-invalid @enderror"
                       value="{{ old('nome', $disciplina->nome) }}">
                @error('nome')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div class="col-md-2">
                <label class="form-label">Código</label>
                <input type="text" name="codigo" class="form-control" value="{{ old('codigo', $disciplina->codigo) }}">
            </div>
            <div class="col-md-3">
                <label class="form-label">Carga Horária (h)</label>
                <input type="number" name="carga_horaria" class="form-control" value="{{ old('carga_horaria', $disciplina->carga_horaria) }}" min="1">
            </div>
            <div class="col-md-2">
                <label class="form-label">Status</label>
                <select name="status" class="form-select">
                    <option value="1" @selected(old('status', $disciplina->status) == 1)>Ativo</option>
                    <option value="0" @selected(old('status', $disciplina->status) == 0)>Inativo</option>
                </select>
            </div>
            <div class="col-12">
                <label class="form-label">Ementa</label>
                <textarea name="ementa" class="form-control" rows="4">{{ old('ementa', $disciplina->ementa) }}</textarea>
            </div>
        </div>
        <div class="d-flex justify-content-end gap-2 mt-4">
            <a href="{{ route('disciplinas.index') }}" class="btn btn-outline-secondary">Cancelar</a>
            <button type="submit" class="btn btn-primary px-4">
                <i class="bi bi-check-lg me-1"></i>Salvar
            </button>
        </div>
    </div>
</form>
@endsection
